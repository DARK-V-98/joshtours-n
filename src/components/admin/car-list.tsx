...
import { deleteCar, toggleCarAvailability } from "@/lib/carActions";
...
export function CarList() {
  ...
  const handleToggleAvailability = (car: AdminCar) => {
    startUpdateTransition(async () => {
      try {
        await toggleCarAvailability(car.id, !car.isAvailable);
        setCars((prev) => prev.map((c) => c.id === car.id ? { ...c, isAvailable: !car.isAvailable } : c));
        toast({ title: "Updated", description: `"${car.name}" is now ${!car.isAvailable ? "enabled" : "disabled"}.` });
      } catch {
                    ...
                    <TableCell className="hidden md:table-cell">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-muted-foreground">
                            {car.isAvailable ? "Enabled" : "Disabled"}
                          </span>
                        </div>
                    </TableCell>
                    <TableCell className="hidden md:table-cell">
