...
function ReviewCard({ review }: { review: GoogleReview }) {
  ...
  return (
    <div className="group flex flex-col gap-4 p-6 rounded-2xl bg-card border border-border hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5 transition-all duration-300">
      {/* Quote icon */}
      ...
      <div className="flex items-center gap-3 pt-2 border-t border-border">
        ...
        {/* Google G logo */}
        <svg viewBox="0 0 24 24" className="w-5 h-5 flex-shrink-0 opacity-50" fill="none">
...
export default function GoogleReviews() {
  ...
  useEffect(() => {
    getCachedReviews().then(({ reviews, meta }) => {
      setReviews(reviews);
      setMeta(meta);
      setLoading(false);
    });
          ...
          {!loading && meta && meta.placeRating > 0 && (
            <div className="flex flex-col items-center gap-2 pt-2">
              <div className="flex items-center gap-3">
                <span className="text-5xl font-display font-bold text-foreground">{meta.placeRating.toFixed(1)}</span>
                <div className="flex flex-col items-start gap-1">
                  <Stars rating={Math.round(meta.placeRating)} size="lg" />
                  <p className="text-sm text-muted-foreground">{meta.totalRatings.toLocaleString()} reviews on Google</p>
                </div>
              </div>
            </div>
          )}
