...
export async function addCar(
  carData: Omit<
    Car,
    'id' | 'createdAt' | 'bookedDates' | 'images' | 'dataAiHint'
  >,
  imageUrls: string[]
) {
  if (!db) {
...
export async function updateCar(
  ...
  try {
    ...
    if (imagesToDelete.length > 0) {
      const deletePromises = imagesToDelete.map((imageUrl) => {
        try {
          ...
          console.error(
            `Failed to create storage reference for URL to delete: ${imageUrl}`,
            error
          );
    ...
    if (typeof updatedData.specifications === 'string') {
      updatedData.specifications = updatedData.specifications
        .split('\n')
        .filter((spec) => spec.trim() !== '');
    }
    
    // The `images` property is handled separately to manage order, additions, and deletions
    delete updatedData.images;


    // 4. Update Firestore document with new data and image order
    const finalImageArray = [...imageOrder, ...newImageUrls].filter(url => !imagesToDelete.includes(url));
...
export async function toggleCarAvailability(carId: string, isAvailable: boolean) {
  ...
  const carDocRef = doc(db, 'cars', carId);
  await updateDoc(carDocRef, { isAvailable });
  revalidatePath('/admin');
  revalidatePath('/cars');
  revalidatePath('/');
}
  ...
  if (carData.images && carData.images.length > 0) {
    ...
    const deletePromises = carData.images.map((imageUrl) => {
      try {
        ...
        console.error(
          `Failed to create storage reference for URL: ${imageUrl}`,
          error
        );
