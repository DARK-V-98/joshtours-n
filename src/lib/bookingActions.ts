...
export async function getConfirmedBookings(): Promise<BookingRequest[]> {
  if (!db) throw new Error("Database not initialized");
  const requestsCollection = collection(db, 'bookingRequests');
  const q = query(requestsCollection, where('status', '==', 'confirmed'), orderBy('createdAt', 'desc'));
  try {
    const querySnapshot = await getDocs(q);
    return querySnapshot.docs.map(doc => {
      const data = doc.data();
      const createdAt = data.createdAt instanceof Timestamp
        ? data.createdAt.toDate().toISOString()
        : new Date().toISOString();
      return { id: doc.id, ...data, createdAt } as BookingRequest;
    });
  } catch {
    return [];
  }
...
export async function getAdminStats(): Promise<AdminStats> {
  if (!db) return { totalBookings: 0, pendingBookings: 0, confirmedBookings: 0, canceledBookings: 0, activeRentals: 0, monthlyBookings: [] };
  try {
    const requestsCollection = collection(db, 'bookingRequests');
    const allDocs = await getDocs(query(requestsCollection, orderBy('createdAt', 'desc')));
    const today = format(new Date(), 'yyyy-MM-dd');

    let pending = 0, confirmed = 0, canceled = 0, active = 0;
    ...
    allDocs.docs.forEach(doc => {
      ...
      if (status === 'pending') pending++;
        ...
        confirmed++;
        if (data.pickupDate <= today && data.returnDate >= today) active++;
    ...
    const monthlyBookings = Array.from({ length: 6 }, (_, i) => {
      ...
      return { month: format(d, 'MMM'), count: monthCounts[key] || 0 };
    });

    return {
      totalBookings: allDocs.size,
    ...
    return { totalBookings: 0, pendingBookings: 0, confirmedBookings: 0, canceledBookings: 0, activeRentals: 0, monthlyBookings: [] };
  }
}
