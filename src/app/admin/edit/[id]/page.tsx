...
import { Loader2, Save, DollarSign, List, Calendar as CalendarIcon, Upload, Trash2, Star } from "lucide-react";
...
export default function EditCarPage() {
  ...
  const handleSetMainImage = (imageUrl: string) => {
    setCurrentImages(prev => [imageUrl, ...prev.filter(url => url !== imageUrl)]);
  };
  ...
  async function onSubmit(values: CarFormValues) {
    ...
    try {
        await updateCar(carId, carDataToUpdate, imagesToDelete, newImages, currentImages);
        toast({
            title: "Success!",
      ...
      toast({
        variant: "destructive",
        title: "Update Error",
        description: "Failed to update the car. Please check your inputs and try again.",
      });
    }
  }
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-display font-bold mb-2">Edit Car</h1>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <Card>
                <CardHeader><CardTitle className="text-lg flex items-center"><List className="mr-2 h-5 w-5"/>Manage Images</CardTitle></CardHeader>
                <CardContent>
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mb-6">
                        {currentImages.map((url, index) => (
                            <div key={url} className="relative group aspect-video">
                                <img src={url} alt="Car image" className="object-contain rounded-md w-full h-full"/>
                                {index === 0 && <Badge className="absolute top-1 right-1 z-10 bg-primary">Main</Badge>}
                                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1">
                                    {index !== 0 && (
                                        <Button type="button" variant="secondary" size="icon" onClick={() => handleSetMainImage(url)} title="Set as main image">
                                            <Star className="h-4 w-4"/>
                                            <span className="sr-only">Set as Main</span>
                                        </Button>
                                    )}
                                    <Button type="button" variant="destructive" size="icon" onClick={() => handleImageDelete(url)} title="Delete image">
